const eventData = {
  ...
  postermaking: ["Poster Presentation"],
  campusxperia: ["campus_xperia"],
  ideathon: ["Ideathon"],
  funwithtech: ["fun with tech"],
  escaperoom: ["escape room"],
  gamengain: ["valorant and bgmi"],
  workshop: ["workshops"],
};
...
$(document).ready(function () {
  ...
  let currentCluster = window.location.href
    .split("/events/")[1]
    .split("/")[0]
    .toLowerCase();
  let initial = true;
  ...
  if ($(window).width() > 768) {
    var $list = $(".cluster-list"),
      clusterMargin =
        $list.css("margin-top") + " " + $list.css("margin-bottom"),
      clusterPadding =
        $list.css("padding-top") + " " + $list.css("padding-bottom"),
      clusterH =
        $list.outerHeight(true) -
        parseInt(clusterPadding) -
        parseInt(clusterMargin),
    ...
    $(".cluster-list > div > a").hover(
      function () {
        ...
        $(this)
          .parent("div")
          .css({ "padding-left": "15%", opacity: 1, transform: "scale(1.1)" });
        $(this)
          .parent("div")
          .next()
          .css({
        ...
        eventData[clusterName].forEach((event) => {
          ...
          eventHTMLString += `
                <div class="event-links">
                    <a href="./${clusterName}/${eventLink}/index.html">${event.toUpperCase()}</a>
                </div>
                `;
        });
    ...
    if (currentCluster !== "") {
      ...
      var elementPosition =
        clusterElement.getBoundingClientRect().top +
        clusterElement.clientHeight / 2;
      ...
      eventData[currentCluster].forEach((event) => {
        ...
        eventHTMLString += `
                <div class="event-links">
                    <a href="./${currentCluster}/${eventLink}/index.html">${event.toUpperCase()}</a>
                </div>
                `;
      });
      $(".event-list").html(eventHTMLString);
    ...
    $(".cluster-list > div").click(function () {
      $(".cluster-list > div")
        .not(this)
        .css({ "padding-left": "5%", opacity: 0.4 });
